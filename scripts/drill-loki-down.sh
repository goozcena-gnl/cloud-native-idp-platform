...
APP_NAME="${APP_NAME:-loki}"
LOCAL_PROMETHEUS_PORT="${LOCAL_PROMETHEUS_PORT:-19105}"
LOCAL_ALERTMANAGER_PORT="${LOCAL_ALERTMANAGER_PORT:-19106}"
ALERT_NAME="${ALERT_NAME:-LokiDown}"
...
detect_loki_workload() {
  if [[ -n "${WORKLOAD_KIND}" && -n "${WORKLOAD_NAME}" ]]; then
    return 0
  fi

  if kubectl -n "${OBS_NAMESPACE}" get statefulset loki >/dev/null 2>&1; then
    WORKLOAD_KIND="statefulset"
    WORKLOAD_NAME="loki"
    return 0
  fi

  if kubectl -n "${OBS_NAMESPACE}" get statefulset loki-single-binary >/dev/null 2>&1; then
    WORKLOAD_KIND="statefulset"
    WORKLOAD_NAME="loki-single-binary"
    return 0
  fi

  if kubectl -n "${OBS_NAMESPACE}" get deployment loki >/dev/null 2>&1; then
  ...
  local detected
  detected="$(kubectl -n "${OBS_NAMESPACE}" get statefulset,deployment \
    -l app.kubernetes.io/name=loki \
    -o jsonpath='{range .items[0]}{.kind}{" "}{.metadata.name}{end}' 2>/dev/null || true)"

  if [[ -n "${detected}" ]]; then
    WORKLOAD_KIND="$(echo "${detected}" | awk '{print tolower($1)}')"
    WORKLOAD_NAME="$(echo "${detected}" | awk '{print $2}')"
    return 0
  fi

  echo "ERROR: Could not detect Loki StatefulSet or Deployment."
  echo
  echo "Available observability workloads:"
  kubectl -n "${OBS_NAMESPACE}" get statefulset,deployment
  exit 1
}
...
restore() {
  ...
  if [[ -n "${ORIGINAL_REPLICAS}" && -n "${WORKLOAD_KIND}" && -n "${WORKLOAD_NAME}" ]]; then
  ...
  echo "Restoring ArgoCD automated sync for loki and idp-root..."
...
wait_for_url() {
  ...
  cat /tmp/drill-loki-down-prometheus.log || true
  echo
  echo "Alertmanager port-forward log:"
  cat /tmp/drill-loki-down-alertmanager.log || true
  exit 1
}
...
wait_for_prometheus_alert_state() {
  ...
  echo "Current LokiDown expression:"
  query_prometheus '(max(up{namespace="observability", service="loki"}) == 0) or absent(up{namespace="observability", service="loki"})' || true
  echo
  echo "Current Loki up:"
  query_prometheus 'up{namespace="observability", service="loki"}' || true
  echo
  echo "Current ALERTS:"
...
echo "Detected Loki workload: ${WORKLOAD_KIND}/${WORKLOAD_NAME}"
echo
...
echo "Initial Loki workload:"
kubectl -n "${OBS_NAMESPACE}" get "${WORKLOAD_KIND}" "${WORKLOAD_NAME}"
echo

echo "Initial Loki pod/service:"
kubectl -n "${OBS_NAMESPACE}" get pod -l app.kubernetes.io/name=loki || true
kubectl -n "${OBS_NAMESPACE}" get svc loki
echo
...
echo "Temporarily disabling ArgoCD automated sync for idp-root and loki..."
kubectl -n "${ARGOCD_NAMESPACE}" patch application idp-root \
  --type=json \
...
kubectl -n "${OBS_NAMESPACE}" port-forward svc/kube-prometheus-stack-prometheus "${LOCAL_PROMETHEUS_PORT}:9090" >/tmp/drill-loki-down-prometheus.log 2>&1 &
PROM_PF_PID=$!

echo "Starting Alertmanager port-forward..."
kubectl -n "${OBS_NAMESPACE}" port-forward svc/kube-prometheus-stack-alertmanager "${LOCAL_ALERTMANAGER_PORT}:9093" >/tmp/drill-loki-down-alertmanager.log 2>&1 &
AM_PF_PID=$!
...
echo "Scaling ${WORKLOAD_KIND}/${WORKLOAD_NAME} to 0 replicas to simulate Loki outage..."
