...
LOCAL_PROMETHEUS_PORT="${LOCAL_PROMETHEUS_PORT:-19101}"
LOCAL_ALERTMANAGER_PORT="${LOCAL_ALERTMANAGER_PORT:-19102}"
ALERT_NAME="${ALERT_NAME:-DemoGrpcDown}"
...
restore() {
  ...
  if [[ -n "${ORIGINAL_REPLICAS}" ]]; then
    echo "Restoring ${APP_NAME} replicas to ${ORIGINAL_REPLICAS}..."
    kubectl -n "${APP_NAMESPACE}" scale "deploy/${APP_NAME}" --replicas="${ORIGINAL_REPLICAS}" >/dev/null 2>&1 || true
  fi

  echo "Restoring ArgoCD automated sync for demo-grpc and idp-root..."
...
wait_for_prometheus_alert_state() {
  ...
  echo "Current alert expression:"
  query_prometheus '(max(up{namespace="apps", service="demo-grpc"}) == 0) or absent(up{namespace="apps", service="demo-grpc"})' || true
  echo
  echo "Current ALERTS:"
...
wait_for_alertmanager_alert() {
  ...
  for attempt in $(seq 1 "${max_attempts}"); do
    ...
    RESPONSE="${response}" ALERT_NAME="${ALERT_NAME}" python - <<'PY'
import json
import os
import sys
...
ORIGINAL_REPLICAS="$(kubectl -n "${APP_NAMESPACE}" get deploy "${APP_NAME}" -o jsonpath='{.spec.replicas}')"
...
echo "Temporarily disabling ArgoCD automated sync for idp-root and demo-grpc..."
kubectl -n "${ARGOCD_NAMESPACE}" patch application idp-root \
  --type=json \
...
kubectl -n "${OBS_NAMESPACE}" port-forward svc/kube-prometheus-stack-prometheus "${LOCAL_PROMETHEUS_PORT}:9090" >/tmp/drill-demo-grpc-down-prometheus.log 2>&1 &
PROM_PF_PID=$!

echo "Starting Alertmanager port-forward..."
kubectl -n "${OBS_NAMESPACE}" port-forward svc/kube-prometheus-stack-alertmanager "${LOCAL_ALERTMANAGER_PORT}:9093" >/tmp/drill-demo-grpc-down-alertmanager.log 2>&1 &
AM_PF_PID=$!
...
echo "Scaling ${APP_NAME} to 0 replicas to simulate outage..."
kubectl -n "${APP_NAMESPACE}" scale "deploy/${APP_NAME}" --replicas=0
kubectl -n "${APP_NAMESPACE}" get deploy "${APP_NAME}"
echo
...
wait_for_prometheus_alert_state "pending" 18
wait_for_prometheus_alert_state "firing" 24
...
echo "Restoring ${APP_NAME} replicas to ${ORIGINAL_REPLICAS}..."
kubectl -n "${APP_NAMESPACE}" scale "deploy/${APP_NAME}" --replicas="${ORIGINAL_REPLICAS}"
