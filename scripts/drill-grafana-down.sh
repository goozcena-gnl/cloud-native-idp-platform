...
APP_NAME="${APP_NAME:-kube-prometheus-stack}"
WORKLOAD_KIND="${WORKLOAD_KIND:-deployment}"
WORKLOAD_NAME="${WORKLOAD_NAME:-kube-prometheus-stack-grafana}"
LOCAL_PROMETHEUS_PORT="${LOCAL_PROMETHEUS_PORT:-19107}"
LOCAL_ALERTMANAGER_PORT="${LOCAL_ALERTMANAGER_PORT:-19108}"
ALERT_NAME="${ALERT_NAME:-GrafanaDown}"
...
restore() {
  ...
  echo "Restoring ArgoCD automated sync for kube-prometheus-stack and idp-root..."
...
wait_for_url() {
  ...
  cat /tmp/drill-grafana-down-prometheus.log || true
  echo
  echo "Alertmanager port-forward log:"
  cat /tmp/drill-grafana-down-alertmanager.log || true
  exit 1
}
...
wait_for_prometheus_alert_state() {
  ...
  echo "Current GrafanaDown expression:"
  query_prometheus '(max(up{namespace="observability", service="kube-prometheus-stack-grafana"}) == 0) or absent(up{namespace="observability", service="kube-prometheus-stack-grafana"})' || true
  echo
  echo "Current Grafana up:"
  query_prometheus 'up{namespace="observability", service="kube-prometheus-stack-grafana"}' || true
  echo
  echo "Current ALERTS:"
...
kubectl -n "${ARGOCD_NAMESPACE}" get applications "${APP_NAME}" idp-root platform-alerts grafana-dashboards \
  -o custom-columns='NAME:.metadata.name,SYNC:.status.sync.status,HEALTH:.status.health.status'
echo

echo "Initial Grafana workload:"
kubectl -n "${OBS_NAMESPACE}" get "${WORKLOAD_KIND}" "${WORKLOAD_NAME}"
echo

echo "Initial Grafana pod/service:"
kubectl -n "${OBS_NAMESPACE}" get pod -l app.kubernetes.io/name=grafana || true
kubectl -n "${OBS_NAMESPACE}" get svc kube-prometheus-stack-grafana
echo
...
echo "Temporarily disabling ArgoCD automated sync for idp-root and kube-prometheus-stack..."
kubectl -n "${ARGOCD_NAMESPACE}" patch application idp-root \
  --type=json \
...
kubectl -n "${OBS_NAMESPACE}" port-forward svc/kube-prometheus-stack-prometheus "${LOCAL_PROMETHEUS_PORT}:9090" >/tmp/drill-grafana-down-prometheus.log 2>&1 &
PROM_PF_PID=$!

echo "Starting Alertmanager port-forward..."
kubectl -n "${OBS_NAMESPACE}" port-forward svc/kube-prometheus-stack-alertmanager "${LOCAL_ALERTMANAGER_PORT}:9093" >/tmp/drill-grafana-down-alertmanager.log 2>&1 &
AM_PF_PID=$!
...
echo "Scaling ${WORKLOAD_KIND}/${WORKLOAD_NAME} to 0 replicas to simulate Grafana outage..."
